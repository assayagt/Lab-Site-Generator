...
class AllWebsitesUserFacade:
    ...
    def create_new_site_manager_from_labWebsite(self, nominator_manager_userId, nominated_manager_email, domain):
        ...
        member = userFacade.get_member_by_email(nominated_manager_email)
        if not member:
            raise Exception(ExceptionsEnum.USER_IS_NOT_A_LAB_MEMBER.value)
        nominated_manager_fullName = member.get_fullName()
        nominated_manager_degree = member.get_degree()
    ...
    def site_creator_resignation_from_lab_website(self, site_creator_user_id, domain, nominate_email, new_role):
        ...
        email = userFacade.get_email_from_token(site_creator_user_id)
        userFacade.error_if_user_is_not_site_creator(email)
