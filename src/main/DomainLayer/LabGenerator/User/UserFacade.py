...
class UserFacade:
    ...
    def remove_site_manager(self, manager_toRemove_email, domain):
        ...
        if not self.dal_controller.members_repo.find_by_email(manager_toRemove_email):
            raise Exception(ExceptionsEnum.USER_IS_NOT_A_LAB_MANAGER.value)
        manager_emails = self.dal_controller.members_repo.find_by_domain
        if manager_toRemove_email not in manager_emails:
            raise Exception(
