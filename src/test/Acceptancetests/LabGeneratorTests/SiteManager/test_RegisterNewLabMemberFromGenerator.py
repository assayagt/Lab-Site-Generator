...
class TestRegisterNewLabMember(unittest.TestCase):
    ...
    def test_register_new_lab_member_success(self):
        ...
        response = self.generator_system_service.register_new_LabMember_from_generator(self.manager_user_id, self.lab_member_email, self.lab_member_full_name, self.domain)
    ...
    def test_user_not_site_manager(self):
        ...
        response = self.generator_system_service.register_new_LabMember_from_generator(self.user_id2, self.lab_member_email, self.lab_member_full_name, self.domain)
        self.assertFalse(response.is_success())
    ...
    def test_email_already_associated_with_a_member(self):
        ...
        response = self.generator_system_service.register_new_LabMember_from_generator(self.manager_user_id, existing_member_email, "Existing Lab Member", self.domain)
        self.assertFalse(response.is_success())
    ...
    def test_email_associated_with_creator_or_manager(self):
        ...
        response = self.generator_system_service.register_new_LabMember_from_generator(self.manager_user_id, "manager1@example.com", "Manager One", self.domain)
        self.assertFalse(response.is_success())
        self.assertEqual(response.get_message(), ExceptionsEnum.EMAIL_IS_ALREADY_ASSOCIATED_WITH_A_MEMBER.value)
